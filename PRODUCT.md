...
- 合同：正式全名、简称、供应商、合同号、精确金额、扫描件/附件/链接、生命周期、多合同节点和逐节点提醒。
- 杂事：名称、一次精确提醒、资料和备注；无紧急度、deadline 或节点。
- L2 默认任务卡片，可切换纯 Agent 对话；L2/L3 共用会话与运行状态，权限和管理控件只在 L3。
- 正式数据以本地 SQLite 为准；归档生成可读快照；飞书只做单向导出。
- Agent 通过 AppCommand、三档权限和授权目录操作应用；可连接用户配置的模型 Provider、检索主工作目录并运行结构化自动化，但无任意 shell、额外网络工具或未授权磁盘权限。
