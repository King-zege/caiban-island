...
**全部里程碑已完成（P0–P10）**：在完整采购任务能力之上，界面已重构为“精密采购工作台”，覆盖 L1/L2/L3、欢迎、新建、草稿、归档、设置、无障碍与发布验收。
...
- 顶部隐藏态提示条只露 4–6px，鼠标停留后连续展开为 L2 横向采购凭条；
- L2 优先呈现下一采购动作、任务、截止风险与压缩节点轨迹，支持键盘和触摸滑动；
- L3 使用 240px 任务栏和单任务工作台，概览、采购节点、资料、提醒、备注一次只显示一个主分区；
- 节点状态显式选择；节点与资料删除延迟 5 秒提交并支持撤销；
