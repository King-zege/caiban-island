...
“精密采购工作台”把采购链路本身作为界面：L1 是低打扰入口，L2 把下一采购动作放在最前，L3 只围绕当前任务展开。采购凭条与由节点、连接线、状态标记组成的采购链路是视觉签名；它们不是装饰，而是行动顺序和风险的共同载体。
...
- **Title**（670，18px，-0.02em）：分区标题、凭条中的下一采购动作。
...
凭条第一行固定为“下一采购动作”，随后是动作标题、任务名、风险/截止和压缩节点轨迹。整张凭条是单一可聚焦按钮，支持左右键、Home/End、触摸拖动和吸附；状态通过短轨迹、图标、文字和可访问名称共同表达。L3 中同一链路展开为节点标记、连接线、状态文字和可编辑选择器，维持从 L2 到 L3 的语义连续。
