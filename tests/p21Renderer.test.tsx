...
describe('P21 统一 Agent 工作区', () => {
  it('L2 默认任务卡片，切换后才显示完整 Agent 功能并可展开同一会话', async () => {
    ...
    expect(screen.getByRole('combobox', { name: 'Agent 权限模式' })).not.toBeNull();
  ...
  it('检测事件序号缺口并从 main 快照补偿暂存流文本', async () => {
    const recovered: AgentRunSnapshot = { sessionId: SESSION.session.id, state: 'running', startedAt: '2026-08-28T00:00:00.000Z', sequence: 3, phase: 'streaming', lastActivityAt: '2026-08-28T00:00:03.000Z', partialText: '从快照恢复的完整文本', activeTool: null, pendingApproval: null, error: null };
