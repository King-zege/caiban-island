...
import type { AgentSessionDetail, AgentSessionSummary, DraftRecord } from '../src/shared/types';
...
beforeEach(() => {
  ...
  useAgentStore.setState({
    sessions: [], detail: null, runState: 'idle', runningSessionId: null,
    streaming: '', activeToolName: null, error: null, drafts: [], memoryProposals: [],
    attention: null, bootstrapped: true
  });
});
...
describe('P14 Agent 工作区', () => {
  it('支持流式文本、取消和会话完成后的恢复', async () => {
    ...
    await act(async () => { useAgentStore.getState().handleEvent({ type: 'text_delta', sessionId: SESSION.id, delta: '正在核对' }); });
    expect(screen.getByText('正在核对')).not.toBeNull();
    await userEvent.click(screen.getByRole('button', { name: '取消' }));
    expect(cancel).toHaveBeenCalledOnce();
    await act(async () => { useAgentStore.getState().handleEvent({ type: 'state', sessionId: SESSION.id, state: 'completed' }); });
  ...
  it('会话列表可切换、导出，且键盘 Enter 可发送', async () => {
    ...
    useAgentStore.setState({ sessions: [SESSION, second], detail: DETAIL });
    render(<AgentPanel />);
  ...
  it('Agent 节点删除提案需要二次确认，并先进入 5 秒撤销队列', async () => {
    const draft: DraftRecord = {
      id: 'draft-delete', source: 'pi', sessionId: 's-delete', state: 'pending', createdAt: '2026-08-23T00:00:00.000Z',
      payload: {
    ...
    const confirm = vi.fn(async () => ({ ok: true as const, data: { type: 'action' as const, taskId: 'task-1' } }));
    setApi({
