...
const ContractLinkSchema = Type.Object({ kind: Type.Union([Type.Literal('url'), Type.Literal('file')]), title: Type.String(), target: Type.String() }, { additionalProperties: false });
const ContractInitialActionSchema = Type.Object({ type: ContractActionTypeSchema, title: Type.String(), description: Type.String(), dueAtUtc: NullableUtc, amountMinor: NullableAmount, remindAtUtc: NullableUtc }, { additionalProperties: false });
const ContractCreateSchema = Type.Object({ ...ContractEditFields, status: EditableContractStatusSchema, initialLinks: Type.Optional(Type.Array(ContractLinkSchema, { maxItems: 20 })), initialActions: Type.Optional(Type.Array(ContractInitialActionSchema, { maxItems: 50 })) }, { additionalProperties: false });
...
const AppCommandSchema = {
  ...Type.Union([
    ...
    Type.Object({ command: Type.Literal('add_contract_link'), input: Type.Object({ contractId: Type.String(), link: ContractLinkSchema }, { additionalProperties: false }) }, { additionalProperties: false }),
