...
export default function NewContractForm({ projects, onClose }: { projects: TaskCard[]; onClose: () => void }): React.JSX.Element {
  ...
  const valid = fullName.trim() && shortName.trim() && supplierName.trim();
  return <Dialog open title="新建合同" description="录入签约后的合同台账；正式全名用于检索和资料管理，简称用于 L2 合同卡。" onClose={onClose}
    ...
    <div className="new-task-form contract-form-grid">
      <Field label="合同正式全名" value={fullName} autoFocus maxLength={500} placeholder="例如：总部办公电脑框架协议采购合同" onChange={(event) => setFullName(event.target.value)} />
      <Field label="合同卡片简称" value={shortName} maxLength={24} hint="最多 24 个字符" placeholder="例如：总部电脑框采合同" onChange={(event) => setShortName(event.target.value)} />
